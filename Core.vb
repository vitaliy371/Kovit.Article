...
Public Class Core
    ...
    Public Shared Sub RegisterRssFeed(ByVal contextPage As Page, ByVal rssUrl As String, ByVal rssTitle As String)
...
#Region "Files Realted"
    Public Shared Function GetFileContents(ByVal FileName As String) As String
        ...
        Try
            Using fs As New IO.FileStream(FileName, IO.FileMode.Open, IO.FileAccess.Read)
                Using sr As StreamReader = New IO.StreamReader(fs)
    ...
    Public Shared Function GetMaxRequestLength() As Integer
        ...
        Dim section As System.Web.Configuration.HttpRuntimeSection = TryCast(ConfigurationManager.GetSection("system.web/httpRuntime"), System.Web.Configuration.HttpRuntimeSection)
...
#End Region
    ...
    Public Shared Function NormalizeForURL(ByRef StringURL As String) As String
        StringURL = StringURL.Replace(" ", "-")
        StringURL = StringURL.Replace(",", "-")

        StringURL = StringURL.Replace(".", "-")
        StringURL = StringURL.Replace("""", "")
        StringURL = StringURL.Replace(Chr(147), "")
        StringURL = StringURL.Replace("#", "")
        StringURL = StringURL.Replace("!", "-")
        StringURL = StringURL.Replace("?", "")
        StringURL = StringURL.Replace("&", "")
        ...
        For index As Integer = 1 To 3
            StringURL = StringURL.Replace("--", "-")
        Next
         If StringURL.EndsWith("-") Then
            StringURL = StringURL.Substring(0, StringURL.Length - 2)
        End If
        If StringURL.StartsWith("-") Then
            StringURL = StringURL.Substring(1, StringURL.Length - 1)
        End If
        Return StringURL
    End Function

    Public Shared Function ValidateEmail(ByVal inputEmail As String) As Boolean
        'my custom regexp!
        Dim strRegex As String = "^([_0-9a-zA-Z]+[-._])*[0-9a-zA-Z_]+@([-0-9a-zA-Z]+[.])+[a-zA-Z]{2,6}$"
        ...
        End If
    End Function
    Public Shared Function ValidateSubject(ByVal text As String) As Boolean
        Dim strRegex As String = "[A-Za-z0-9_][A-Za-z_0-9]*"
        Dim re As New Regex(strRegex)
